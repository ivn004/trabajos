...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/sucursales':
			...
			let salas = theaters.map(function(salas){
				return salas.total_rooms
			})
			let totalSalas = salas.reduce(function(acum,salas){
				return acum + salas
			})
			res.write("Total de salas:" + totalSalas + "\n\n") 
